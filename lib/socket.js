...
module.exports = function(redis) {
  ...
  return function(socket) {
    ...
    socket.on('subscribe on service', function(serviceClient, serviceSubscribe, alias, meta_target) {
      ...
      redis.getRegistrations({
        ...
        if (services.length > 0) {
          ...
          socket.to(serviceClient.id).emit('service subscribed', services[0], alias, meta_target);
        }
